...
DB_CONFIGS["rds"] = json.loads(_secret["SecretString"])
...
def seed_customers(cur, idx):
    ...
    buf.seek(0)
    cur.copy_expert(
        "COPY customers (customer_id, name, email, city, status) FROM STDIN WITH CSV",
        buf,
    )
...
def seed_products(cur, idx):
    ...
    buf.seek(0)
    cur.copy_expert(
        "COPY products (product_id, product_name, description, price, is_active) FROM STDIN WITH CSV",
        buf,
    )
...
def seed_orders(cur, idx):
    ...
    buf.seek(0)
    cur.copy_expert(
        "COPY orders (order_id, customer_id, order_date, order_status, total_amount) FROM STDIN WITH CSV",
        buf,
    )
...
def seed_order_items(cur, idx):
    ...
    buf.seek(0)
    cur.copy_expert(
        "COPY order_items (order_item_id, order_id, product_id, quantity, unit_price) FROM STDIN WITH CSV",
        buf,
    )
